...
export class ContactService {
  //mock the server
  private _contactsDb: ContactModel[] = CONTACTS;
  ...
  public loadContacts(filterBy = null): void {
    ...
    if (
      filterBy &&
      (filterBy.term.length || filterBy.phone.length || filterBy.email.length)
    ) {
  ...
  private _sort(contacts: ContactModel[]): ContactModel[] {
  ...
  private _filter(contacts, filterBy) {
    const term = filterBy.term.toLocaleLowerCase();
    const phone = filterBy.phone.toLocaleLowerCase();
    const email = filterBy.email.toLocaleLowerCase();
    return contacts.filter((contact) => {
      return (
        contact.name.toLocaleLowerCase().includes(term) &&
        contact.phone.toLocaleLowerCase().includes(phone) &&
        contact.email.toLocaleLowerCase().includes(email)
      );
    });
  }
